...
# Check if eas is installed and add to PATH if necessary
if ! command -v eas >/dev/null 2>&1; then
  echo "eas CLI not found in PATH. Please install eas CLI or add it to your PATH."
  echo "You can install it via npm: npm install -g eas-cli"
  exit 1
fi
...
for app in "${APPS[@]}"; do
  echo "=== Starting build for $app (android, production) ==="
  (
    just build "$app" android production
