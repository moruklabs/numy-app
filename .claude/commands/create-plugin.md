...
When creating components, execute independent operations in parallel:

- Multiple skill files -> ONE message with multiple Write calls
- Multiple command files -> ONE message with multiple Write calls
- Multiple validation checks -> ONE message with multiple Bash calls

### Depth-First Strategy (DFS over BFS)

When implementing, go DEEP into one component type before moving to the next:

- Complete all skills fully before moving to commands
- Finish one component's full specification before starting another

### Architecture Principles

**FSD over DDD:** Structure plugin components following Feature-Sliced Design patterns.

**TDD First:** Include validation and testing for each component.

**DRY & OPEN-CLOSED:**

- Check existing plugins for patterns to reuse
