...
When creating multiple rules, write ALL in parallel:

- Multiple rule files -> ONE message with multiple Write calls

### Depth-First Strategy (DFS over BFS)

When creating rules, go DEEP into one rule before moving to the next:

- Complete one rule's full specification before starting another
- Test one rule's pattern fully before designing the next

### Architecture Principles

**DRY & OPEN-CLOSED:**

- Check existing rules for patterns to reuse
- Design patterns for extension (use regex groups)
- Avoid overlapping rules that trigger multiple times
