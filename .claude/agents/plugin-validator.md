...
**CRITICAL: Run ALL validation checks in a SINGLE message.**

Execute ALL file discovery in parallel:

```
Bash: cat .claude-plugin/plugin.json | jq .
Glob: commands/**/*.md
Glob: agents/**/*.md
Glob: skills/*/SKILL.md
Bash: cat hooks/hooks.json 2>/dev/null
```

Then run ALL frontmatter checks in parallel:

```
Grep: "^---" in commands/**/*.md
Grep: "^name:" in agents/**/*.md
Grep: "^description:" in commands/**/*.md
```

**Validation Process:**

1. **Locate Plugin Root (ALL VALIDATION IN PARALLEL)**:
